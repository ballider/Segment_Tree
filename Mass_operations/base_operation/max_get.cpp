...
struct segment_tree{
    ...
    void max(const int64_t l, const int64_t r, const int64_t v){
        max(l, r, v, 0, 0, size);
    }

    void max(const int64_t L, const int64_t R, const int64_t v, int64_t x, int64_t left, int64_t right){
        ...
        max(L, R, v, 2*x+1, left, m);
        max(L, R, v, 2*x+2, m, right);
    }
    ...
    int64_t get(const int64_t i, int64_t x, int64_t left, int64_t right){
        ...
        int64_t m = (left+right)/2;
        if(i < m) ans = std::max(ans, get(i, 2*x+1, left, m));
        else ans = std::max(ans, get(i, 2*x+2, m, right));
